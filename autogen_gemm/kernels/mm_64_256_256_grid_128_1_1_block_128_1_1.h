...
extern "C" __global__ void __launch_bounds__(128)
    mm_64_256_256(float* __restrict__ A, float* __restrict__ B,
  ...
  for (int k_outer = 0; k_outer < 8; ++k_outer) {
    __syncthreads();
    A_shared[(((int)threadIdx.x))] = A[((((((((int)blockIdx.x) >> 4) * 2048) +
                                           ((((int)threadIdx.x) >> 5) * 256)) +
                                          (k_outer * 32)) +
                                         (((int)threadIdx.x) & 31)))];
    A_shared[((((int)threadIdx.x) + 128))] =
        A[(((((((((int)blockIdx.x) >> 4) * 2048) +
               ((((int)threadIdx.x) >> 5) * 256)) +
              (k_outer * 32)) +
             (((int)threadIdx.x) & 31)) +
            1024))];
  ...
  compute[((
      ((((((int)blockIdx.x) >> 4) * 2048) + ((((int)threadIdx.x) >> 4) * 256)) +
       ((((int)blockIdx.x) & 15) * 16)) +
