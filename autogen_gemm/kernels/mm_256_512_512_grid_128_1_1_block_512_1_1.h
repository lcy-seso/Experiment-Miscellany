...
extern "C" __global__ void __launch_bounds__(512)
    mm_256_512_512(float* __restrict__ A, float* __restrict__ B,
  ...
  for (int k_outer = 0; k_outer < 16; ++k_outer) {
    __syncthreads();
    A_shared[(((int)threadIdx.x))] = A[((((((((int)blockIdx.x) >> 4) * 16384) +
                                           ((((int)threadIdx.x) >> 5) * 512)) +
                                          (k_outer * 32)) +
                                         (((int)threadIdx.x) & 31)))];
    A_shared[((((int)threadIdx.x) + 512))] =
        A[(((((((((int)blockIdx.x) >> 4) * 16384) +
               ((((int)threadIdx.x) >> 5) * 512)) +
              (k_outer * 32)) +
             (((int)threadIdx.x) & 31)) +
            8192))];
  ...
  compute[((((((((int)blockIdx.x) >> 4) * 16384) +
              ((((int)threadIdx.x) >> 5) * 512)) +
             ((((int)blockIdx.x) & 15) * 32)) +
